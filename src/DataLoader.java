...
//               "src/res/winequality-white.csv" // <- pass på at filen ligger i riktig fil bane
                "res/winequality-red.csv",
                "res/winequality-white.csv"
        ...
        for (String filename : files) {
            try (InputStream is = DataLoader.class.getResourceAsStream(filename);
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    String[] columns = line.split(";");
                    String alcStr = columns[10].replace("\"", "").trim();
                    uniqueValues.add(Double.parseDouble(alcStr));
                }

            } catch (IOException e) {
                System.err.println("Error reading: " + filename);
            } catch (NullPointerException e) {
                System.err.println("File not found on classpath: " + filename);
            }
        }

        double[] result = new double[uniqueValues.size()];
        int i = 0;
        for (double val : uniqueValues) result[i++] = val;
        return result;
    }
}
